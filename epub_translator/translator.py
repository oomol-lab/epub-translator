...
from .epub2 import HTMLFile
from .llm import LLM
from .translation import Fragment, Incision, Language, ProgressReporter
...
class _Translator:
    ...
    def do(self, source_path: Path, translated_path: Path, working_path: Path | None) -> None:
        ...
        try:
            ...
            context.replace_ncx(
                lambda texts: self._translate_ncx(
                    texts=texts, cache_path=cache_path, report_progress=lambda p: self._report_progress(p * 0.1)
                ),
            )
