...
def translate(source_path: Path, target_path: Path):
    ...
    with Zip(source_path, target_path) as zip:
        ...
        for chapter_path in search_spine_paths(zip):
            ...
            _translate_chapter(translator, chapter)
...
def _translate_chapter(translator: Translator, chapter: Chapter):
    for paragraph, translated_element in zip(
        chapter.paragraphs,
        split(
            segments=(TruncatableXML(p.clone_raw()) for p in chapter.paragraphs),
